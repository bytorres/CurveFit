...
#Collect the Gene Expression for that Stage
#rec_patients<-row.names(d)
#Rec_Exp<-Exp1[,rec_patients]
...
recsnp_idx<-which(data2_add@gtdata@idnames %in% patients)
rec_snp_correct<-as.numeric(data2_add@gtdata[recsnp_idx,])
dorder<-match(rec_patients,row.names(rec_snp_correct))
Test<-rec_snp_correct[dorder,]
write.table(Test,'rec_snp_ordered.txt',sep='\t')
sel_resid_file_name=paste('rec_snp_ordered.txt') 
...
meq_sel_recovery= Matrix_eQTL_engine(
  snps = sel,  
  gene = gene_sel,
  cvrt = ncvrt, 
...
View(meq_sel_recovery$all$eqtls)
sel_hits<-meq_sel_recovery$all$eqtls
write.table(sel_hits,'sel_hits_10.txt',sep="\t")


sel_snps<-as.character(meq_sel_recovery$all$eqtls$snps)
sel_snps<-unique(sel_snps)
snp_list<-data2_add@gtdata@snpnames
sel_idx<-which(snp_list %in% sel_snps)
sel_snp_gtype<-as.numeric(data2_add[,sel_idx])
View(sel_snp_gtype)
colnames(sel_snp_gtype)[1] ==sel_snps[1]
sel_snp_gtype<-sel_snp_gtype[which(row.names(sel_snp_gtype) %in% ctrl_id),]
...
#sel_resid_gene_fix<-meq_sel_recovery_fix$all$eqtls$gene
